globals.__n_sh = @get_shell
globals.__n_rt = @get_router
globals.__n_lb = @include_lib
metaxploit = globals.__n_lb("/lib/metaxploit.so"); if not metaxploit then exit("Motor Nini: falta metaxploit.so")
__nini_enjambre = []
...
__nini_crackear_hash = function(user_or_hash, h=null)
  c = globals.__n_lb("/lib/crypto.so")
...
__nini_obtener_info_libreria = function(ip, p)
  m = globals.__n_lb("/lib/metaxploit.so")
...
__nini_descubrir_red_local = function()
  c = globals.__n_sh().host_computer
  lip = c.local_ip
  parts = lip.split("\.")
...
__nini_replicar_binario = function(s, r)
  ...
  p = globals.__n_sh().host_computer.File(program_path)
  if not p then return false
  res = s.scp(p.path, r,globals.__n_sh(), 1)
...
__nini_conectar_ssh = function(ip, user, pass, port=22)
  return globals.__n_sh().connect_service(ip, port, user, pass)
end function
__nini_conectar_ftp = function(ip, user, pass)
  return globals.__n_sh().connect_service(ip, 21, user, pass, "ftp")
end function
...
__nini_scp_upload = function(shell, filePath, remotePath)
  ...
  local = globals.__n_sh().host_computer.File(filePath)
  if not local then return false
  return shell.scp(filePath, remotePath,globals.__n_sh(), 1)
end function
__nini_scp_download = function(shell, remotePath, localPath)
  if not shell or typeof(shell) != "shell" then return false
  return shell.scp(remotePath, localPath,globals.__n_sh(), 1)
end function
...
__nini_get_router_from_node = function(shell)
  ...
  return globals.__n_rt(local_ip)
end function
...
__nini_limpiar_logs = function(shell=null)
  ...
  if not shell then shell = globals.__n_sh()
  comp = shell.host_computer
...
Buffer.max_size = 50
    ...
    if len(Buffer.items) >= Buffer.max_size then
    ...
    if index < 0 or index >= len(Buffer.items) then
    return null
end if
        return Buffer.items[index].objeto
end function
    Buffer.list = function()
    if len(Buffer.items) == 0 then
...
Cob = {}
    ...
    if globals.custom_object.hasIndex(clave) then
    return globals.custom_object[clave]
end if
        return null
end function
    Cob.del = function(clave)
    if globals.custom_object.hasIndex(clave) then
    globals.custom_object.remove(clave)
end if
end function
Clipboard = {}
...
Clipboard.gamma = null
    ...
    if espacio == "a" then
    Clipboard.alpha = valor
end if
    if espacio == "b" then
    Clipboard.beta = valor
end if
    if espacio == "c" then
    Clipboard.gamma = valor
end if
end function
    Clipboard.get = function(espacio)
    if espacio == "a" then
    return Clipboard.alpha
end if
    if espacio == "b" then
    return Clipboard.beta
end if
    if espacio == "c" then
    return Clipboard.gamma
end if
        return null
end function
Aliases = {}
...
Aliases.custom = {}
    ...
    if Aliases.custom.hasIndex(entrada) then
    return Aliases.custom[entrada]
end if
    if Aliases.lista.hasIndex(entrada) then
    return Aliases.lista[entrada]
end if
        return entrada
end function
...
Kernel.prompt_completo = true
    ...
    Kernel.info = function()
        ...
        ["Modo", Kernel.modo,],
        ["Sesiones", str(len(globals.enjambre)),],
...
Config.datos = {"color_primario": "#00e5ff", "auto_jump": true, "max_buffer": 50}
    ...
    contenido = contenido + clave + "=" + str(Config.datos[clave]) + char(10)
        ...
        archivo = globals.__n_sh().host_computer.File(Config.archivo)
    if archivo then
    ...
    if contenido == null or contenido == "" then
    return
end if
    ...
    continue
            ...
            partes = linea.split("=")
        if len(partes) >= 2 then
    Config.datos[partes[0].trim] = partes[1:].join("=").trim
end if
end for
end function
    Config.obtener = function(clave, default)
    if Config.datos.hasIndex(clave) then
    return Config.datos[clave]
end if
        return default
end function
...
Log.maximo = 100
    ...
    if len(Log.historial) > Log.maximo then
    Log.historial.remove(0)
end if
end function
    Log.ver = function(cantidad)
        cantidad = cantidad or 20
        if len(Log.historial) > cantidad then
    inicio = len(Log.historial) - cantidad
        else
    inicio = 0
        end if
    ...
    if not comp or not ruta then
    return null
end if
    if typeof(comp) == "shell" then
    comp = comp.host_computer
end if
    if typeof(comp) != "computer" then
    return null
end if
        es_local = comp.public_ip == globals.__n_sh().host_computer.public_ip and comp.local_ip == globals.__n_sh().host_computer.local_ip
    if es_local then
            archivo = comp.File(ruta)
        if archivo then
    return archivo
    ...
    for parte in partes
        if parte == "" or parte == "." then
    continue
end if
            encontrado = null
            hijos = actual.get_folders + actual.get_files
        for h in hijos
            if h.name == parte then
                    encontrado = h
                    break
end if
end for
    ...
    Kernel.inicializar = function()
        Config.cargar()
    if not globals.hasIndex("custom_object") then
    globals.custom_object = {}
end if
    if not globals.hasIndex("enjambre") then
    globals.enjambre = {}
end if
...
Glasspool.activo = false
    ...
    if not Glasspool.activo or Glasspool.sesion == null then
    return null
end if
    if typeof(Glasspool.sesion) == "shell" then
    return Glasspool.sesion.launch("/bin/bash", "-c '" + comando + "'")
end if
        return null
end function
...
Shell.comandos = {}
    ...
    return funcion()
        ...
        if len(args) == 1 then
    return funcion(args[0])
end if
        if len(args) == 2 then
    return funcion(args[0], args[1])
end if
        if len(args) == 3 then
    return funcion(args[0], args[1], args[2])
end if
        if len(args) == 4 then
    return funcion(args[0], args[1], args[2], args[3])
end if
            return funcion(args.join(" "))
end if
    if nombre == "ls" or nombre == "dir" then
    return Shell.cmd_ls(args)
end if
    if nombre == "cd" then
    return Shell.cmd_cd(args)
end if
    if nombre == "cat" then
    return Shell.cmd_cat(args)
end if
    if nombre == "pwd" then
    return Shell.cmd_pwd()
end if
    if nombre == "whoami" then
    return Shell.cmd_whoami()
end if
    if nombre == "clear" or nombre == "limpiar" then
    return Shell.cmd_clear()
end if
    if nombre == "exit" or nombre == "break" then
    return "EXIT"
end if
    ...
    if nueva_ruta == ".." then
        ...
        if Kernel.current_path == "" then
    Kernel.current_path = "/"
end if
            return Kernel.current_path
end if
    if nueva_ruta[0] != "/" then
    nueva_ruta = Kernel.current_path + "/" + nueva_ruta
end if
...
Prompt = {}
    Prompt.generar = function()
        ip_pub = globals.__n_sh().host_computer.public_ip
        ip_local = globals.__n_sh().host_computer.local_ip
    ...
    if Kernel.usuario == "root" then
    return Colores.rojo + "root$ " + Colores.reset
end if
...
Menu = {}
    ...
    return null
        ...
        idx = val(sel) - 1
    if idx < 0 or idx >= len(opciones) then
    return null
end if
    if opciones[idx].hasIndex("cmd") then
    return opciones[idx].cmd
end if
        return idx
end function
Progress = {}
...
Status.info = function(t); return Colores.cian + "* " + Colores.reset + t; end function
    ...
    if len(encontrados) > 0 then
    Buffer.push(encontrados, "Red " + ip_base)
end if
        return encontrados
end function
    whois_objetivo = function(ip)
        router = globals.__n_rt(ip)
    if not router then
    return null
end if
    ...
    puertos_abiertos.push(str(port.port_number))
end if
end for
        filas = [
    ...
    for v in vuln.v_list
        ...
        if sesion != null then
            ...
            if tipo == "shell" or tipo == "computer" then
                ...
                if Config.obtener("auto_jump", true) then
    jumping_auto(sesion, ip)
end if
                    return sesion
end if
end if
    ...
    __nini_limpiar_logs = function(ip)
    if ip == "" then
            archivo = Kernel.obtener_archivo(globals.__n_sh().host_computer, "/var/system.log")
        if archivo then
    archivo.set_content("")
end if
            print("<color=#00e5ff>[#] </color><color=#ffffff>" + "Logs locales limpiados" + "</color>")
    else if globals.enjambre.hasIndex(ip) then
    ...
    jump_manual = function()
    if len(globals.enjambre) == 0 then
    return
end if
    ...
    return
        ...
        idx = val(sel) - 1
    if idx < 0 or idx >= len(nodos) then
    return
end if
        jumping_auto(globals.enjambre[nodos[idx]], nodos[idx])
end function
    ...
    if len(globals.enjambre) == 0 then
            print("<color=#ff1744>[X] </color><color=#ffffff>" + "El globals.enjambre esta vacio. Asalta algum objetivo primero." + "</color>")
            return
end if
        print("<color=#00e5ff>[#] </color><color=#ffffff>" + "SESIONES ACTIVAS (" + str(len(globals.enjambre)) + "):" + "</color>")
        nodos = globals.enjambre.indexes
        columnas = ["#", "IP", "Tipo"]
        filas = []
    for i in range(0, len(nodos))
    ...
    if idx < 0 or idx >= len(nodos) then
        ...
        sesion_seleccionada = globals.enjambre[ip_seleccionada]
        modulo_sesion_interactuar(ip_seleccionada, sesion_seleccionada)
end function
    modulo_sesion_interactuar = function(ip, sesion)
        tipo = typeof(sesion)
    ...
    if accion == "terminal" then
    sesion.start_terminal
    else if accion == "files" then
    sesion_action_files(ip, sesion)
    else if accion == "info" then
    sesion_action_info(ip, sesion)
    else if accion == "download" then
    sesion_action_download(ip, sesion)
    else if accion == "upload" then
    sesion_action_upload(ip, sesion)
    else if accion == "exec" then
    sesion_action_exec(ip, sesion)
    else if accion == "adduser" then
    sesion_action_adduser(ip, sesion)
    else if accion == "view" then
    sesion_action_view_file(ip, sesion)
    else if accion == "list" then
    sesion_action_list_dir(ip, sesion)
    else if accion == "ports" then
    sesion_action_ports(ip, sesion)
    else if accion == "devices" then
    sesion_action_devices(ip, sesion)
    else if accion == "jump" then
    sesion_action_jump(ip, sesion)
    else if accion == "scanjump" then
    sesion_action_scanjump(ip, sesion)
end if
end function
    sesion_action_jump = function(ip, sesion)
    if typeof(sesion) != "shell" then
    ...
    sesion_action_scanjump = function(ip, sesion)
    if typeof(sesion) != "shell" then
    ...
    sesion_action_files = function(ip, sesion)
        comp = null
    if typeof(sesion) == "shell" then
    comp = sesion.host_computer
    ...
    listar_archivos = function(archivo, indent)
    if not archivo then
    return
end if
        nombre = archivo.name
    ...
    nombre = "/"
        ...
        tipo_arch = ""
    if archivo.is_folder then
    tipo_arch = "<color=#00e5ff>[DIR]</color>"
    else if archivo.is_binary then
    tipo_arch = "<color=#ffea00>[BIN]</color>"
    else; tipo_arch = "<color=#ffffff>[FILE]</color>"
end if
        permisos = ""
    if archivo.has_permission("r") then
    permisos = permisos + "r"
end if
    if archivo.has_permission("w") then
    permisos = permisos + "w"
end if
    if archivo.has_permission("x") then
    permisos = permisos + "x"
end if
    ...
    sesion_action_info = function(ip, sesion)
    ...
    sesion_action_download = function(ip, sesion)
        comp = null
    if typeof(sesion) == "shell" then
    comp = sesion.host_computer
    else if typeof(sesion) == "computer" then
    ...
    if contenido == null then
        ...
        archivo_local = Kernel.obtener_archivo(globals.__n_sh().host_computer, ruta_l)
    if archivo_local then
            archivo_local.set_content(contenido)
    else
            globals.__n_sh().host_computer.touch(local_dir, nombre_local)
            archivo_local = Kernel.obtener_archivo(globals.__n_sh().host_computer, ruta_l)
        if archivo_local then
    archivo_local.set_content(contenido)
end if
    ...
    sesion_action_upload = function(ip, sesion)
        comp = null
    if typeof(sesion) == "shell" then
    comp = sesion.host_computer
    else if typeof(sesion) == "computer" then
    ...
    return
        ...
        archivo_local = globals.__n_sh().host_computer.File(ruta_local)
    ...
    sesion_action_exec = function(ip, sesion)
    if typeof(sesion) != "shell" then
    ...
    sesion_action_adduser = function(ip, sesion)
        comp = null
    if typeof(sesion) == "shell" then
    comp = sesion.host_computer
    else if typeof(sesion) == "computer" then
    ...
    if resultado == 1 then
    exito("Usuario creado: " + usuario)
    else; error("No se pudo crear el usuario.")
end if
end function
    sesion_action_view_file = function(ip, sesion)
    ...
    sesion_action_list_dir = function(ip, sesion)
    if typeof(sesion) != "file" or not sesion.is_folder then
            print("<color=#ff1744>[X] </color><color=#ffffff>" + "Esta accion requiere un directorio." + "</color>")
            return
end if
        hijos = sesion.get_files
    if not hijos or len(hijos) == 0 then
    ...
    for hijo in hijos
            tipo = "FILE"
        if hijo.is_folder then
    tipo = "DIR"
end if
        if hijo.is_binary then
    tipo = "BIN"
end if
    ...
    sesion_action_ports = function(ip, sesion)
    ...
    sesion_action_devices = function(ip, sesion)
    if typeof(sesion) != "number" and typeof(sesion) != "router" then
            print("<color=#ff1744>[X] </color><color=#ffffff>" + "Esta accion requiere un router." + "</color>")
            return
end if
        devices = sesion.devices
    if not devices or len(devices) == 0 then
...
"root123", "toor", "changeme", "default",
    ...
    for sample in samples
            mayus = sample.upper
        if len(mayus) <= order then
    siguiente
end if
        for i in range(0, len(mayus) - order - 1)
                token = mayus[i : i + order]
                next_char = mayus[i + order]
            if not chains.hasIndex(token) then
    chains[token] = []
end if
            if chains[token].indexOf(next_char) == null then
    chains[token].push(next_char)
end if
end for
end for
end function
    ...
    return null
        ...
        return chains[token][floor(rnd * len(chains[token]))]
end function
    ...
    if ip == "" or ip == null then
            return globals.__n_sh(user, password)
end if
        return globals.__n_sh().connect_service(ip, port, user, password)
end function
    chainsaw_crack = function(sesion, user, ip, port)
    ...
    if globals.objetivo_actual == null then
            ...
            sesion = globals.__n_sh
            ip = ""
            port = 0
    else
        if not globals.enjambre.hasIndex(globals.objetivo_actual) then
    ...
    if accion == "analizar" then
    modulo_escalada_vector(sesion)
    ...
    else if accion == "brute" then
    modulo_escalada_brute(sesion, "root", ip, port)
end if
end function
    modulo_escalada_vector = function(sesion)
    ...
    if suids then
        for f in suids
            if f.is_binary then
    vectores.push({"path": f.path, "type": "SUID", "severity": "MEDIA"})
end if
end for
end if
        etc = comp.File("/etc")
    ...
    vectores.push({"path": f.path, "type": "WRITEABLE_ETC", "severity": "ALTA"})
end if
end for
end if
    if len(vectores) == 0 then
            ...
            print("<color=#00ff41>[OK] </color><color=#ffffff>" + "Vectores: " + str(len(vectores)) + "</color>")
            columnas = ["Ruta", "Tipo", "Severidad"]
            filas = []
        for v in vectores
                color = "<color=#00ff41>"
            if v.severity == "ALTA" then
    color = "<color=#ff3131>"
            else if v.severity == "MEDIA" then
    color = "<color=#ffea00>"
end if
    ...
    modulo_escalada_brute = function(sesion, user, ip, port)
    ...
    if globals.modo == "backdoor" then
    persistencia_instalar_backdoor()
    else if globals.modo == "cron" then
    persistencia_cron()
    else if globals.modo == "list" then
    persistencia_ver()
end if
end function
    ...
    if not comp then
    error("No hay sesión."); return
end if
        lib_folder = comp.File("/lib")
    if not lib_folder or not lib_folder.has_permission("w") then
    error("Sin permisos root."); return
end if
    ...
    if not cron or not cron.has_permission("w") then
    error("Sin permisos."); return
end if
    ...
    if len(globals.enjambre) == 0 then
    error("Enjambre vacío."); return
end if
        print("<color=#00e5ff>[#] </color><color=#ffffff>" + "Instalando in " + str(len(globals.enjambre)) + " nodos..." + "</color>")
...
_version = "STABLE"
    inicializar_vault = function()
        p = "/var/revenant"
    if not globals.__n_sh().host_computer.File(p) then
    p = "./"
end if
        ruta = p + "/vault.db"
    if globals.__n_sh().host_computer.File(ruta) then
    ...
    cifrar = function(txt)
        ...
        for i in range(0, len(txt) - 1)
            v = __nini_xor(code(txt[i]), code(_vault_k[i % len(_vault_k)]))
            res = res + str(v) + ","
end for
        return res
end function
    ...
    continue
            ...
            res = res + char(__nini_xor(val(p), code(_vault_k[len(res) % len(_vault_k)])))
end for
        return res
end function
    ...
    p = "/home/" + active_user
        ...
        f = globals.__n_sh().host_computer.File(ruta)
    if not f then
            globals.__n_sh().host_computer.touch(p, "vault.db")
            f = globals.__n_sh().host_computer.File(ruta)
end if
    if f then
    ...
    if not globals.objetivo_actual then
    error("Fije un objetivo primero."); return
end if
        comp = obtener_computadora_objetivo()
    if not comp then
    error("No hay sesión activa."); return
end if
    ...
    if not carpeta or not carpeta.is_folder then
    return resultados
end if
    if carpeta.get_folders then
        for fd in carpeta.get_folders
            if prof > 0 then
    resultados = resultados + intel_buscar_en_directorio(comp, fd.path, exts, prof - 1)
end if
end for
end if
...
if (globals.modo == "keys") then
...
else if (globals.modo == "passwords") then
    exts = [".kdb", ".kdbx", ".1pif", ".psafe3"]
else if (globals.modo == "configs") then
    ...
    else; info("Encontrados: " + str(len(resultados))); dibujar_ventana("RESULTADOS", str(resultados))
end if
end function
    ...
    if globals.modo == "ssh" then
    info("SSH brute not disponible in GH. Usa Metaxploit.")
    else if globals.modo == "ftp" then
    intel_ftp_anonimo()
    else if globals.modo == "passwd" then
    intel_crack_passwd()
end if
end function
    ...
    if not host then
    host = __nini_pedir("Host FTP: "); if host == "" then  return
end if
    ...
    if metax then
        ...
        if sesion then
    globals.enjambre[host] = sesion; exito("Acceso anonimo!"); return
end if
end if
    ...
    error("No hay sesión."); return
        ...
        passwd = comp.File("/etc/passwd")
    if not passwd or passwd.is_binary then
    error("No se pudo leer."); return
end if
    ...
    if globals.modo == "download" then
            modulo_exfil_download()
    else if globals.modo == "upload" then
                modulo_exfil_upload()
    else if globals.modo == "shell" then
                    modulo_exfil_reverse_shell()
    else if globals.modo == "payload" then
    ...
    if sesion then
        ...
        if archivo then
                contenido = archivo.get_content
                print("<color=#00e5ff>[#] </color><color=#ffffff>" + "Archivo descargado: " + len(contenido) + " bytes" + "</color>")
